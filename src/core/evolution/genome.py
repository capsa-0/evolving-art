...
# Assuming that 'shapes' is an available module
from src.core.shapes import Shape, UnitDisk, UnitSquare, Polygon, UnionN, IntersectionN, Difference
...
class PrimitiveNode:
    ...
    # --- CHANGE HERE ---
        return {"primitives": 1, "operations": 0}
...
class OpNode:
    ...
    def size(self) -> dict[str, int]:
    # --- CHANGE HERE ---
        primitives_count = sum(c.size()["primitives"] for c in self.children)
        operations_count = 1 + sum(c.size()["operations"] for c in self.children)
        return {"primitives": primitives_count, "operations": operations_count}
